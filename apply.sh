...
original_author="zhanghulu2004"
original_name="tmh_test"
original_urlname="TMH_test"
original_description="Awesome tmh_test created by zhanghulu2004" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/tmh_test "${name}"
...
then
    ...
    echo "then run 'tmh_test' or 'python -m tmh_test'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'tmh_test' or 'python -m tmh_test' or 'poetry run tmh_test'"
fi
